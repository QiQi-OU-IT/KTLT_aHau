...
// Ham tinh tich hai ma tran vuong
void tinhTichMaTran(int** a, int** b, int** c, int n) {
    // Duyet qua tung phan tu cua ma tran ket qua c
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            c[i][j] = 0; // Khoi tao gia tri cua phan tu c[i][j]
            // Tinh tich cho phan tu c[i][j]
            for (int k = 0; k < n; k++) {
                c[i][j] += a[i][k] * b[k][j];
            }
        }
