...
// Ham tinh tong cac so duong trong 1 dong cua ma tran
int tinhTongSoDuongTrenDong(int** a, int n, int dong) {
    int tong = 0;

    // Duyet qua tat ca cac phan tu trong dong
    for (int j = 0; j < n; j++) {
        if (a[dong][j] > 0) {  // Kiem tra neu phan tu la so duong
            tong += a[dong][j]; // Cong phan tu vao tong
        }
    }

    return tong; // Tra ve tong cac so duong tren dong
}
...
int main() {
    ...
    // Tinh tong cac so duong trong dong
    int tong = tinhTongSoDuongTrenDong(a, n, dong);
    cout << "Tong cac so duong trong dong " << dong << " la: " << tong << endl;
