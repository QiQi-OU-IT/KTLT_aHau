...
// Ham sap xep cac phan tu trong 1 dong theo thu tu tang dan
void sapXepDongTangDan(int** a, int n, int dong) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (a[dong][j] > a[dong][j + 1]) {
                // Hoan doi a[dong][j] va a[dong][j+1]
                int temp = a[dong][j];
                a[dong][j] = a[dong][j + 1];
                a[dong][j + 1] = temp;
            }
...
int main() {
    ...
    // Sap xep cac phan tu trong dong
    sapXepDongTangDan(a, n, dong);

    cout << "Ma tran sau khi sap xep dong " << dong << " theo thu tu tang dan: " << endl;
