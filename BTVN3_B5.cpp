...
// Ham xoa phan tu o dau mang
void xoaPhanTuDauMang(int*& arr, int& n) {
    ...
    // Sao chep cac phan tu tu vi tri 1 tro di vao mang moi
    for (int i = 0; i < n - 1; i++) {
        newArr[i] = arr[i + 1];
    }
...
int main() {
    ...
    xoaPhanTuDauMang(arr, n);

    // Xuat mang sau khi xoa phan tu
    cout << "Mang sau khi xoa phan tu o dau: ";
    xuatMang(arr, n);
