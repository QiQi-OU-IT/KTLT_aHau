...
// Ham tinh tong hai ma tran vuong
void tinhTongMaTran(int** a, int** b, int** c, int n) {
    // Duyet qua tung phan tu cua hai ma tran a va b
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            c[i][j] = a[i][j] + b[i][j]; // Cong ma tran
        }
    }
}
