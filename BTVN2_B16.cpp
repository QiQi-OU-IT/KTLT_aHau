...
// Ham kiem tra 1 dong trong ma tran co tang dan khong
bool kiemTraDongTangDan(int** a, int m, int n, int dong) {
    // Duyet qua dong va kiem tra xem phan tu sau co lon hon phan tu truoc hay khong
    for (int i = 1; i < n; i++) {
        if (a[dong][i] <= a[dong][i - 1]) {
            return false; // Neu phan tu sau khong lon hon phan tu truoc, tra ve false
        }
    }
    return true; // Dong tang dan
}
...
int main() {
    ...
    // Kiem tra dong co tang dan khong
    if (kiemTraDongTangDan(a, m, n, dong)) {
        cout << "Dong " << dong << " co tang dan!" << endl;
    }
    else {
        cout << "Dong " << dong << " khong tang dan!" << endl;
    }
