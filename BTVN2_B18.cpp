...
int tinhTongSoDuong(int** a, int m, int n) {
    int tong = 0;

    // Duyet qua tat ca cac phan tu trong ma tran
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if (a[i][j] > 0) {  // Kiem tra neu phan tu la so duong
                tong += a[i][j]; // Cong phan tu vao tong
            }
        }
    }

    return tong; // Tra ve tong cac so duong
}
...
int main() {
    ...
    // Tinh tong cac so duong trong ma tran
    int tong = tinhTongSoDuong(a, m, n);
    cout << "Tong cac so duong trong ma tran: " << tong << endl;
